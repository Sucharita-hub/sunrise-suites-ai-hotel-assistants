...
export async function answerQuestion({ message, history = [] }) {
  ...
  if (!client) {
    return {
      answer: deterministicAnswer(message, safeHistory),
...
If the guest asks about availability, tell them you can check it after receiving check-in, check-out and guest count.
...
${hotelContext}`
  ...
  try {
    ...
    return {
      answer: text || deterministicAnswer(message, safeHistory),
      source: provider
    };
  } catch (error) {
